...
workflow-ordering gap prevents coroutine-based I/O from using stateful
allocators: `operator new` runs before the receiver exists, so
the allocator arrives too late. This gap is architectural, not
a missing feature that can be extended stably without
destabilising the API. We recommend
...
route; unfortunately, the authors have been unable to find a
way to close this gap.

The allocator timing gap is not a missing feature that can be
...
The allocator timing gap has three facets: the cost of
...
backward from receiver to sender. This creates an ordering gap for
coroutine frame allocation:
<!-- JSL:
...
allocator object as leading arguments. A coroutine that
naturally takes a single argument becomes a three-parameter
call, two-thirds of which is allocator plumbing rather than
application logic - creating strong pressure to simply omit
the allocator and fall back to the global heap.
...
present again. The difference is that the concerns which were
speculative in 2022 are now concrete: specific LWG issues,
neutral committee polls, and unresolved NB comments document
the gaps rather than merely anticipate them.
...
allocator timing gap should be resolved before the
...
the allocator timing gap. We welcome any solution that closes
...
timing, allocator propagation, and the interaction with
...
the result was a stronger proposal built on the foundations
already laid. The same precedent applies here: the P2300
...
premature standardisation imposes a concrete cost on the
use case with the broadest demonstrated demand.
...
active change is healthy for a maturing design, but it
signals that the work is not yet finished.
...
**We ask the committee to address the allocator timing gap
...
The complete timing sequence showing the gap between frame
...
receiver's allocator can be used. There is no timing gap.
...
the same timing mismatch, but coroutines make it pervasive:
...
the allocator timing issue, the algorithm customisation issue,
